...
const word maskStop = 0x8070;

//Frame alignment, sequence and mask
const word sequenceFrame = 0x8000;
...
void endTx(unsigned long lastCycles)
{
  ...
  while (cyclesCurrent - cyclesTx < WAIT_CYCLES_TX)
    {
      cyclesCurrent = ARM_DWT_CYCCNT;
    }
...
void loop() // run over and over
{
  ...
  //Check receive line
    ...
    if (!receptionIsActive)
      ...
      else 
      {
        ...
        if (indexTx > 0 && (halfBitsDataTx[indexTx-1] & maskStop) == sequenceEnd)
        {
          //No more reception
      ...
      /*
        //Parity error detection, commented out as it is too slow
        if (indexTx > 0)
        {
  ...
  */
        ...
        {
          digitalWriteFast(PIN_OVERFLOW, HIGH);
          Serial.println("[DEBUG] ERROR MAX FRAMES");
          receptionIsActive=false;
          waitForResponse=false;
        }

        //Detection of incorrect frame alignment, light LED
        if (indexTx > 0 && (halfBitsDataTx[indexTx-1] & maskFrame) != sequenceFrame)
        {
          digitalWriteFast(PIN_OVERFLOW, HIGH);
          //Serial.println("[DEBUG] SYNC ERROR");
          receptionIsActive=false;
          waitForResponse=false;
        }

        /*Detect incorrect intrabit transitions, commented out
    ...
    if (indexTx >0)
    {
      ...
      for (int i = 0; i < indexTx; i++) {  
        ...
        if (ENABLEDEBUG) Serial.print("[DEBUG] "); 
        if (ENABLEDEBUG) Serial.println(halfBitsDataTx[i], BIN);
